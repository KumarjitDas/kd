/**
 * @file kdGenMemOpsFindBlockIndicesRangeU64.c
 * @author Kumarjit Das
 * @brief KD_GEN_MEM_OPS library kdGenMemOpsFindBlockIndicesRangeU64 function test.
...
#define TEST_NAME_CSTR  LIB_NAME_CSTR " library kdGenMemOpsFindBlockIndicesRangeU64 function test"
...
BasicArguments(void)
{
    ...
    status = GenMemOpsFindBlockIndicesRangeU64(null, sizeof(idxs), &found, buf, USIZE_C(16), 0, 16, blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* found pointer is null -> failure */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), null, buf, USIZE_C(16), 0, 16, blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* ptr is null -> failure */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, null, USIZE_C(16), 0, 16, blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* block is null -> failure */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(16), 0, 16, null, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* idxs_sz is zero -> success but found=0 */
    found  = 55;
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, USIZE_C(0), &found, buf, USIZE_C(16), 0, 16, blk, USIZE_C(4));
    assert(status == RESULT_SUCCESS);
    assert(found == 0);

    /* ptr_sz is zero -> success, found=0 */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(0), 0, 0, blk, USIZE_C(4));
    assert(status == RESULT_SUCCESS);
    assert(found == 0);

    /* block_sz is zero -> failure */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(16), 0, 16, blk, USIZE_C(0));
    assert(status == RESULT_FAILURE);

    /* idxs_sz smaller than sizeof(u64) -> success but found=0 */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, USIZE_C(7), &found, buf, USIZE_C(16), 0, 16, blk, USIZE_C(4));
    assert(status == RESULT_SUCCESS);
    assert(found == 0);

    /* begin > end -> failure */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(16), 10, 5, blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* end > ptr_sz -> failure */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(16), 0, 17, blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);
...
RangeConstraints(void)
{
     ...
     */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(32), 8, 20, blk, USIZE_C(4));
    ...
    assert(idxs[0] == U64_C(8));
    assert(idxs[1] == U64_C(12));
    assert(idxs[2] == U64_C(16));
...
CapacityLimit(void)
{
    ...
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(16), 0, 16, blk, USIZE_C(2));
...
UnalignedMatchFail(void)
{
     ...
     */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(8), 1, 6, blk, USIZE_C(2));
     ...
     */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(8), 0, 6, blk, USIZE_C(2));
    assert(status == RESULT_SUCCESS);
    assert(found == 2);
    assert(idxs[0] == U64_C(2));
...
LargeCount(void)
{
    ...
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, USIZE_C(2000), 0, 2000, blk, USIZE_C(1));
...
U16_FindBlockIndicesRange(void)
{
     ...
     */
    status   = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, sizeof(buf), 10, 1900, blk, sizeof(u16));

    assert(status == RESULT_SUCCESS);

    /* Found 1 block. Total bytes = 2. */
    assert(found == 2);
    assert(idxs[0] == U64_C(1800));
...
U32_FindBlockIndicesRange(void)
{
    ...
    status   = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, sizeof(buf), 0, 4000, blk, sizeof(u32));

    assert(status == RESULT_SUCCESS);
    assert(found == 8);
    assert(idxs[0] == U64_C(4));
    assert(idxs[1] == U64_C(3996));
...
U64_FindBlockIndicesRange(void)
{
     ...
     */
    status = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, sizeof(buf), 16, 4000, blk, sizeof(u64));

    assert(status == RESULT_SUCCESS);
    assert(found == 8);
    assert(idxs[0] == U64_C(16));
...
Struct_FindBlockIndicesRange(void)
{
     ...
     * Excludes 0.
     */
    status  = GenMemOpsFindBlockIndicesRangeU64(idxs, sizeof(idxs), &found, buf, sizeof(buf), 24, 2400, &blk, sz);

    assert(status == RESULT_SUCCESS);

    /* Found 2 blocks (24, 2376). Total bytes = 48. */
    assert(found == 48);
    assert(idxs[0] == U64_C(24));
    assert(idxs[1] == U64_C(2376));
