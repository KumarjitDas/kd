/**
 * @file kdGenMemOpsReplaceLastBlock.c
 * @author Kumarjit Das
 * @brief KD_GEN_MEM_OPS library kdGenMemOpsReplaceLastBlock function test.
...
#define TEST_NAME_CSTR  LIB_NAME_CSTR " library kdGenMemOpsReplaceLastBlock function test"
...
BasicArguments(void)
{
    ...
    /* ptr is null -> failure */
    status = GenMemOpsReplaceLastBlock(null, USIZE_C(16), find_blk, new_blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* ptr_sz is zero -> failure */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(0), find_blk, new_blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* find_block is null -> failure */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(16), null, new_blk, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* new_block is null -> failure */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(16), find_blk, null, USIZE_C(4));
    assert(status == RESULT_FAILURE);

    /* block_sz is zero -> failure */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(16), find_blk, new_blk, USIZE_C(0));
    assert(status == RESULT_FAILURE);

    /* block_sz > ptr_sz -> failure */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(16), find_blk, new_blk, USIZE_C(17));
    assert(status == RESULT_FAILURE);
...
ReplaceNone(void)
{
    ...
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(16), find_blk, new_blk, USIZE_C(4));
...
ReplaceLastOfMultiple(void)
{
    ...
    /* Replace LAST */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(16), find_blk, new_blk, USIZE_C(2));

    assert(status == RESULT_SUCCESS);

    /* Verify last occurrence (offset 10) replaced */
    assert(memcmp(buf + 10, new_blk, 2) == 0);

    /* Verify earlier occurrences (2, 6) unchanged */
    assert(memcmp(buf + 2, find_blk, 2) == 0);
...
ReplaceAtBoundaries(void)
{
    ...
    /* Case 1: Start of buffer (Only occurrence) */
    kdi_FillVal(buf, USIZE_C(16), U8_C(0x00));
...
ReplaceSameValue(void)
{
    ...
    kdi_FillVal(buf + 4, 2, 0x55);

    kdi_FillVal(val, 2, 0x55);

    /* Replace 0x5555 with 0x5555 */
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(8), val, val, USIZE_C(2));

    assert(status == RESULT_SUCCESS);
    assert(memcmp(buf + 4, val, 2) == 0);
...
ReplaceBlock_U16(void)
{
    ...
    status = GenMemOpsReplaceLastBlock(buf, sizeof(buf), &find_blk, &new_blk, sizeof(u16));

    assert(status == RESULT_SUCCESS);

    /* Verify index 5 replaced (Last) */
    assert(buf[5] == 0xBBBB);

    /* Verify index 2 unchanged */
    assert(buf[2] == 0xAAAA);
...
ReplaceBlock_U32(void)
{
    ...
    status = GenMemOpsReplaceLastBlock(buf, sizeof(buf), &find_blk, &new_blk, sizeof(u32));

    assert(status == RESULT_SUCCESS);
    assert(buf[7] == 0x98765432);
    assert(buf[3] == 0x12345678);
...
ReplaceBlock_U64(void)
{
    ...
    status = GenMemOpsReplaceLastBlock(buf, sizeof(buf), &find_blk, &new_blk, sizeof(u64));

    assert(status == RESULT_SUCCESS);

    /* Last index 9 should change */
    assert(buf[9] == new_blk);

    /* First index 1 should remain */
    assert(buf[1] == find_blk);
...
ReplaceBlock_Struct(void)
{
    ...
    /* Place target at index 1 and index 4 */
    buf[1]     = find_blk;
    buf[4]     = find_blk;

    status     = GenMemOpsReplaceLastBlock(buf, sizeof(buf), &find_blk, &new_blk, sizeof(kdi_TestStruct));

    assert(status == RESULT_SUCCESS);

    /* Verify replacement at last index (4) */
    assert(memcmp(&buf[4], &new_blk, sizeof(kdi_TestStruct)) == 0);

    /* Verify index 1 untouched */
...
PartialMatchIgnored(void)
{
    ...
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(10), find_blk, new_blk, USIZE_C(3));
...
LargeBuffer(void)
{
    ...
    status = GenMemOpsReplaceLastBlock(buf, USIZE_C(2000), find_blk, new_blk, USIZE_C(5));

    assert(status == RESULT_SUCCESS);

    /* Verify 1000 changed (Last) */
    assert(memcmp(buf + 1000, new_blk, 5) == 0);

    /* Verify 100 unchanged (First) */
